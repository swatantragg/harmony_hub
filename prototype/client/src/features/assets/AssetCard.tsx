...
// Image assets show their real bytes, fetched through a presigned inline URL. Everything
  ...
  return (
    ...
    >
      ...
      <div className="card-body">
        ...
        <div className="card-key" title={asset.s3.key}>{midTruncate(asset.s3.key, 20, 12)}</div>
        <div className="card-foot">
          <AvailabilityBadge status={asset.availability.status} />
          <span className="t-small" style={{ fontFamily: 'var(--mono)', fontSize: 11 }}>{bytes(asset.s3.sizeBytes)}</span>
        </div>
      </div>
    </button>
  );
...
export function AssetRow({ asset, onOpen, selected }: { asset: Asset; onOpen: (a: Asset) => void; selected?: boolean }) {
  ...
  return (
    <tr className={selected ? 'selected' : ''} onClick={() => onOpen(asset)}>
      ...
      <td className="t-small" style={{ fontFamily: 'var(--mono)' }}>{bytes(asset.s3.sizeBytes)}</td>

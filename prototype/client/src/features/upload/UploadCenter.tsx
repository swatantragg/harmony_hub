...
import { checksum, runUpload, useQueue } from './useUploadQueue';
          ...
          <div className="stack-3">
            {items.map((item) => (
              <UploadRow
                ...
                onStart={() => start(item)}
                onPause={() => pause(item)}
                onRemove={() => { controllers.current.get(item.id)?.abort(); remove(item.id); }}
              />
            ))}
          </div>
        </>
      ...
      <div className="panel-body stack-3">
          ...
          <div className="stack-2">
            ...
            <div className="spread">
              <span className="t-small">
                {item.state === 'FINALISING' ? 'Assembling and verifying in storage…' : `${item.progress}% · ${bytes(item.bytesSent)} of ${bytes(item.file.size)}`}
              </span>
              <span className="t-small">
                {item.partStates.filter((p) => p === 'done').length} / {item.partStates.length} parts
              </span>
            </div>
            {item.partStates.length > 1 && (
              <div className="parts" aria-hidden>
                {item.partStates.map((p, i) => <i key={i} className={p} />)}
              </div>
            )}
          </div>
        )}
        ...
        {item.state === 'FAILED' && (
          <div className="note danger">
            ...
            <div><b>Upload stopped.</b> {item.error} — press Resume to retry only the parts that did not land.</div>
          </div>
        )}
        ...
        {complete && item.result && (
          <div className="note ok">
            ...
            <div className="grow">
              <b>In storage and verified.</b> Stored as{' '}
              <span className="keytext">{item.result.s3.key}</span>
              {item.result.folderName && (
                <div className="t-small" style={{ marginTop: 4 }}>
                  <FolderIcon size={11} style={{ verticalAlign: -1 }} /> Grouped under “{item.result.folderName}” in the catalogue.
                </div>
              )}
            </div>
            {item.result.songId ? (

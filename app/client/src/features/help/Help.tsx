// "How Harmony Hub works" — the whole product explained in one scrollable page, in plain
...
const CONCEPTS = [
  ...
  },
  {
    ...
    body: 'Files travel from your browser directly into storage without passing through the Harmony Hub API. Big files are split into chunks that upload four at a time; if one chunk fails it retries by itself, and you can pause and resume without losing progress.',
  },
  {
    icon: History,
...
const LINK_TYPES = [
  {
    ...
    body: 'No account, no sign-in, no Harmony Hub login screen. This is the link for press, DSPs, sync agencies and anyone outside the company. Control comes from the expiry, the download cap and the revoke button — not from who the visitor is.',
  },
  {
    icon: PenLine,
    title: 'Editor',
    who: 'Signed-in Harmony Hub accounts that can edit',
...
const DELETE_LEVELS = [
  {
    ...
    points: [
      ...
      'Recoverable for 30 days from either side — Harmony Hub or drive.google.com. After that Google deletes it for good.',
  ...
  },
  {
    ...
    points: [
      'Every file inside is moved back to the library root, in Harmony Hub and in Google Drive together.',
    ...
    <div className="page stack-5" style={{ maxWidth: 900 }}>
      <div className="page-head">
        ...
        <p className="t-body" style={{ fontSize: 17, maxWidth: '58ch' }}>
          Harmony Hub keeps every file made for a release — masters, covers, videos, reels, lyric sheets —
      ...
      <section>
        ...
        <p className="t-body" style={{ marginBottom: 18, maxWidth: '62ch' }}>
          This is the only vocabulary Harmony Hub asks you to learn. Every file carries exactly one of
      ...
      <section>
        ...
        <div className="note indigo" style={{ marginTop: 16 }}>
          <Link2 size={15} />
          <div>
            <b>What the link actually is.</b> A Harmony Hub address — <span className="t-mono">/#/s/&lt;token&gt;</span> — never
            a Google Drive address. It carries a random token and nothing else: no file id, no folder, no
            account. When someone opens it, Harmony Hub re-checks the expiry, the revocation, the download

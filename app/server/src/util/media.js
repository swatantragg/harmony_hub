...
export function wav({ seconds = 6, seed = 'x', sampleRate = 8000 }) {
  ...
  const swing = 1.5 + ((h >> 7) % 13) / 10;
  const timbre = 0.12 + ((h >> 11) % 23) / 100;

  for (let i = 0; i < n; i += 1) {
    const t = i / sampleRate;
    const step = Math.floor(t * swing) % notes.length;
    const f = root * notes[(step + rotate) % notes.length];
    const env = Math.min(1, t * 4) * Math.max(0, 1 - (t % 0.5) * 1.4);
    const s = Math.sin(2 * Math.PI * f * t) * 0.5 + Math.sin(2 * Math.PI * f * 2 * t) * timbre;

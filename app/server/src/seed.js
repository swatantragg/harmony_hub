...
export async function seed({ log = console.log } = {}) {
  ...
  db.artists.forEach((artist) => {
    ...
    put.stage({
      ...
      appProperties: properties({
        app: 'gcloud', assetId, artist: artist.name, assetType: 'Artist Photo', family: 'Image',
      }),
    });
  ...
  if (reel) {
    ...
    put.stage({
      ...
      appProperties: properties({ app: 'gcloud', assetId: oldId, version: 'V1' }),
    });
  ...
  db.unfiled = unfiledSpecs.map(([displayName, type, mime, tags, folderIdx, description], i) => {
    ...
    put.stage({
      ...
      contentType: mime,
      appProperties: properties({
        app: 'gcloud', assetId, assetType: type, family: familyOf(type) || 'Document',
        tags: tags.join(', '),
      }),
    });
  ...
  if (sourceReel) {
    ...
    put.stage({
      ...
      body: dupBody,
      contentType: 'video/mp4',
      appProperties: properties({ app: 'gcloud', assetId: sourceReel.assetId }),
    });
    put.stage({
      asset: identical,
      body: dupBody,
    ...
    put.stage({
      ...
      body: nearBody,
      contentType: 'video/mp4',
      appProperties: properties({ app: 'gcloud', assetId: nearId }),
    });
  ...
  if (mismatch?.drive.fileId) {
    ...
    mismatch.availability = {
      ...
      checkMethod: 'LIST_RECONCILE',
      detail: 'The file was changed in Google Drive outside GCloud (size drift)',
    };
    drift.push('MISMATCH');
  }
